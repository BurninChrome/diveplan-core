...
def initialize():
    ...
    parser.add_argument('-g', '--gas', dest='gases', nargs='+',
                        help='gas used for this dive, format : [depth],[o2],[he]; default is air for all the dive. ex: -g 90,14,58 for a trimix used at 60m with 14%% O2 and 58%% He')

    parser.add_argument('-o', '--deco', dest='decos', nargs='+',
                        help='gas used for decompression, format : [depth],[o2],[he],[time]; ex: -o 20,50,0 for a 50%% nitrox for deco')
    ...
    if args.gases:
        for gas in args.gases:
            ...
            if len(splitedgas) == 3:
                g = {}
                g['depth'] = float(splitedgas[0])
                g['o2'] = float(splitedgas[1]) / 100
                g['he'] = float(splitedgas[2]) / 100
                gases.append(g)
            else:
