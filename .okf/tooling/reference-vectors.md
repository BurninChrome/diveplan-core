...
475 checks across nine files, generated by `test/vectors/gen_vectors.py` — an
...
```
ndl-air.tsv    12 checked   10 deviating   12 allowed   FIXED - update expected-failures.txt
```
...
**Coverage of the constant tables is uneven.** `zh_l16A` is checked against the
formula that defines it; `zh_l16C` against a retyped published table held
independently of `src/`. **`zh_l16B` and `zh_l12` are checked against nothing** —
neither could be sourced to that standard, and inventing a reference is the
defect this directory was rewritten to remove. For those two, `test_profiles`
is the only net. See [`zh_l12` unverified](/findings/zh-l12-unverified.md).

**One row of the published reference is weaker than the rest.** The published
ZH-L16C table has 16 compartments starting at 5.0 min; this library's has 17,
including a 4-minute one. That row's values are the formula results rather than
a transcription, and it drives a deviation in the less conservative direction.
Flagged in `gen_vectors.py`.
